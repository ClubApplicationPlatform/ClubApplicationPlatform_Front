...
export function SignupPage() {
  ...
  useEffect(() => {
    ...
    const remain = Math.max(0, Math.floor((saved.expiresAt - Date.now()) / 1000));
    if (remain > 0) {
  ...
  const handleSendVerificationCode = async () => {
    ...
    if (!validateSchoolEmail(formData.email)) {
      setErrors((prev) => ({
        ...prev,
        email: "학교 이메일만 사용할 수 있습니다. (@st.yc.ac.kr)",
      }));
      return;
    }
    ...
    if (stored && stored.email === formData.email) {
      ...
      if (since < RESEND_COOLDOWN) {
        toast.error(`잠시 후 다시 시도해주세요. (${RESEND_COOLDOWN - since}s)`);
        return;
      }
    }
  ...
  const handleVerifyCode = () => {
    ...
    if (!saved || saved.email !== formData.email) {
      toast.error("인증 정보를 찾을 수 없습니다. 다시 전송해주세요.");
      return;
    }
    if (Date.now() > saved.expiresAt) {
      toast.error("인증 시간이 만료되었습니다. 다시 전송해주세요.");
      setTimeLeft(0);
      clearCodeFromSession();
      return;
    }
    ...
    <div className="container mx-auto flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-12">
      <Card className="w-full max-w-md">
        <CardHeader>
          ...
          <CardDescription>JoinUs와 함께 동아리 활동을 시작해보세요.</CardDescription>
        </CardHeader>
        <CardContent>
